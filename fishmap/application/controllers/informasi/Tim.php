...
class Tim extends ApplicationBase
{
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        // load model
        $this->load->model('informasi/M_tim');
    ...
    public function index()
    {
        ...
        $this->smarty->assign("template_content", "informasi/tim/index.html");
        // load js                                       
        ...
        $search = $this->tsession->userdata('search_data_tim');
        ...
        $config['base_url'] = site_url("informasi/tim/index/");
        $config['total_rows'] = $this->M_tim->get_total_tim($params);
        ...
        $rs_id = $this->M_tim->get_tim($params, $limit);
    ...
    public function add()
    {
        ...
        $rs = $this->M_tim->insert($params);
        $post_id = $this->db->insert_id();
        redirect('informasi/tim/edit/' . $post_id . '/add');
    }
    ...
    public function edit($post_id = null, $form = "")
    {
        ...
        // set template content
        $this->smarty->assign("template_content", "informasi/tim/edit.html");
        // load js                              
        ...
        // post
        $result = $this->M_tim->get_by_id($post_id);
    ...
    public function search_process()
    {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('search_data_tim');
        } else {
            $params = array(
                "post_title" => $this->input->post("post_title"),
                "post_date"  => $this->input->post("post_date")
            );
            $this->tsession->set_userdata('search_data_tim', $params);
        }
        // default redirect
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function edit_process()
    {
        ...
        $this->tnotification->set_rules('post_title', 'Nama', 'trim|required');
        $this->tnotification->set_rules('post_content', 'Konten', 'trim|required');        
        $this->tnotification->set_rules('name_penulis', 'Nama Instansi', 'trim|required');
        $this->tnotification->set_rules('profile_penulis', 'Status', 'trim|required');
        $this->tnotification->set_rules('post_link', 'Tautan', 'trim|required');
        ...
        if ($this->tnotification->run() !== false) {
            ...
            $params = array(
                ...
                'post_link'         => $this->input->post('post_link', TRUE),                                                
                'name_penulis'      => $this->input->post('name_penulis'),
                'profile_penulis'   => $this->input->post('profile_penulis'),                                                            
                'post_date'         => $this->input->post('post_date'),
            ...
            if ($_FILES['photo_penulis']['name']) {
                $file           = self::_upload_file('photo_penulis'); // file_name                                                                            
                $photo_penulis     = self::PATH_FILE_PUBLIC . ($file['file_name'] ?? 'ajax-loader.gif');
                if (($photo_penulis)) {
                    $params['photo_penulis'] = $photo_penulis;
                }
            }
            ...
            // update tim
            if ($this->M_tim->update($params, $where)) {
                // parameters file
                if ($file) {
                    $params2 = array(
    ...
    public function delete_process($post_id = null)
    {
        ...
        $rs_informasi = $this->M_tim->delete($params);
    ...
    public function delete_img($post_id = null)
    {
        ...
        $rs_informasi = $this->M_tim->update(['post_image' => NULL, 'updated_by' => $this->com_user['user_id']], $params);
        if ($rs_informasi) {
            $this->tnotification->sent_notification("success", "Data berhasil dihapus");
        } else {
            $this->tnotification->sent_notification("error", "Data gagal dihapus");
    ...
    private function _update_img($id = '', $path = null)
    {
        ...
        if (empty($id)) {
            ...
            $rs = $this->M_tim->update(['post_image' => $path, 'updated_by' => $this->com_user['user_id']], ['post_id' => $id]);
        }
    }
