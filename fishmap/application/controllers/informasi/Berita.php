...
class Berita extends ApplicationBase
{
    ...
    public function index()
    {
        ...
        $this->smarty->assign("template_content", "informasi/berita/index.html");
        // load js                                       
        ...
        $search = $this->tsession->userdata('search_data_berita');
        ...
        $config['base_url'] = site_url("informasi/berita/index/");
        $config['total_rows'] = $this->M_berita->get_total_berita($params);
        ...
        $rs_id = $this->M_berita->get_berita($params, $limit);
    ...
    public function add()
    {
        ...
        // create new row
        $this->smarty->assign("template_content", "informasi/berita/add.html");
        $params = array(
        ...
        $rs = $this->M_berita->insert($params);
        $post_id = $this->db->insert_id();
        redirect('informasi/berita/edit/' . $post_id . '/add');
    }
    ...
    public function edit($post_id = null, $form = "")
    {
        ...
        // set template content
        $this->smarty->assign("template_content", "informasi/berita/edit.html");
        // load js                             
        ...
        // post
        $result = $this->M_berita->get_by_id($post_id);
    ...
    public function search_process()
    {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('search_data_berita');
        } else {
            $params = array(
                "post_title" => $this->input->post("post_title"),
                "post_date"  => $this->input->post("post_date")
            );
            $this->tsession->set_userdata('search_data_berita', $params);
        }
        // default redirect
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function edit_process()
    {
        ...
        if ($this->tnotification->run() !== false) {
            ...
            // update berita
            if ($this->M_berita->update($params, $where)) {
                // parameters file
                if ($file) {
                    $params2 = array(
    ...
    public function delete_process($post_id = null)
    {
        ...
        $rs_informasi = $this->M_berita->delete($params);
    ...
    public function delete_img($post_id = null)
    {
        ...
        $rs_informasi = $this->M_berita->update(['post_image' => NULL, 'updated_by' => $this->com_user['user_id']], $params);
        if ($rs_informasi) {
            $this->tnotification->sent_notification("success", "Data berhasil dihapus");
        } else {
            $this->tnotification->sent_notification("error", "Data gagal dihapus");
    ...
    private function _update_img($id = '', $path = null)
    {
        ...
        if (empty($id)) {
            ...
            $rs = $this->M_berita->update(['post_image' => $path, 'updated_by' => $this->com_user['user_id']], ['post_id' => $id]);
        }
    }
