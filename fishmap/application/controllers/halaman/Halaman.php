...
class Halaman extends ApplicationBase
{
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        // load model
        $this->load->model('halaman/M_halaman');
    ...
    public function index()
    {
        ...
        $this->smarty->assign("template_content", "halaman/halaman/index.html");
        // get search parameter
        $search = $this->tsession->userdata('search_data_halaman');
        ...
        // pagination                
        $params = [
            'post_category' => 'halaman',
            $search_param['post_title'],
            $search_param['post_date']
        ];
        $config['base_url'] = site_url("halaman/halaman/index/");
        $config['total_rows'] = $this->M_halaman->get_total_halaman($params);
        ...
        $rs_id = $this->M_halaman->get_halaman($params, $limit);
    ...
    public function add()
    {
        ...
        $rs = $this->M_halaman->insert($params);
        $post_id = $this->db->insert_id();
        redirect('halaman/halaman/edit/' . $post_id . '/add');
    }
    ...
    public function edit($post_id = null, $form = '')
    {
        ...
        // set template content
        $this->smarty->assign("template_content", "halaman/halaman/edit.html");
        // load js                  
        ...
        // post
        $result = $this->M_halaman->get_by_id($post_id);
    ...
    public function search_process()
    {
        ...
        if ($this->input->post('save') == "Reset") {
            $this->tsession->unset_userdata('search_data_halaman');
        } else {
            $params = array(
                "post_title" => $this->input->post("post_title"),
                "post_date"  => $this->input->post("post_date")
            );
            $this->tsession->set_userdata('search_data_halaman', $params);
        }
        // default redirect
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function add_process()
    {
        ...
        if ($this->tnotification->run() !== false) {
            ...
            $post_type       = 'post';
            $post_link       = NULL;
            $last_order['post_order'] = $this->M_halaman->get_last_order();
            if (!empty($last_order['post_order'])) {
                foreach ($last_order['post_order'] as $value) {
                    $value++;
                }
            } else {
                $value = 1;
            ...
            // insert
            if ($this->M_halaman->insert($params)) {
                // notification
    ...
    public function edit_process()
    {
        ...
        if ($this->tnotification->run() !== false) {
            ...
            // update artikel
            if ($this->M_halaman->update($params, $where)) {
                // parameters file
                if ($file) {
                    $params2 = array(
    ...
    public function delete_process($post_id = null)
    {
        ...
        $rs = $this->M_halaman->delete($params);
        if ($rs) {
    ...
    public function delete_img($post_id = null)
    {
        ...
        $rs_informasi = $this->M_halaman->update(['post_image' => NULL, 'updated_by' => $this->com_user['user_id']], $params);
        if ($rs_informasi) {
            $this->tnotification->sent_notification("success", "Data berhasil dihapus");
        } else {
            $this->tnotification->sent_notification("error", "Data gagal dihapus");
    ...
    private function _update_img($id = '', $path = null)
    {
        ...
        if (empty($id)) {
            ...
            $rs = $this->M_halaman->update(['post_image' => $path, 'updated_by' => $this->com_user['user_id']], ['post_id' => $id]);
        }
    }
