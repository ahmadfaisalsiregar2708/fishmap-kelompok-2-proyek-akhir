...
class Adminportal extends ApplicationBase
{
    ...
    public function process_add()
    {
        ...
        $this->_set_page_rule("C");
        // cek input
        $this->tnotification->set_rules('portal_nm', 'Nama Portal', 'trim|required|max_length[100]');
        ...
        if ($this->tnotification->run() !== false) {
            $params = [$this->input->post('portal_nm'), $this->input->post('site_title'), $this->input->post('site_desc'), $this->input->post('meta_desc'), $this->input->post('meta_keyword'), $this->com_user['user_id'], self::generateRandomString(20)];
            // insert
    ...
    public function process_update()
    {
        ...
        $this->_set_page_rule("U");
        // cek input
        $this->tnotification->set_rules('portal_id', 'Group ID', 'trim|required|max_length[11]');
        $this->tnotification->set_rules('portal_nm', 'Nama Portal', 'trim|required|max_length[100]');
        $this->tnotification->set_rules('site_title', 'Judul Web', 'trim|required|max_length[100]');
        $this->tnotification->set_rules('site_desc', 'Deskripsi Web', 'trim|required|max_length[100]');
        $this->tnotification->set_rules('meta_desc', 'Meta Deskripsi', 'trim|required|max_length[255]');
        ...
        if ($this->tnotification->run() !== false) {
            $params = [
                $this->input->post('portal_nm'),
                $this->input->post('site_title'),
                $this->input->post('site_desc'),
                $this->input->post('meta_desc'),
    ...
    public function process_delete()
    {
        ...
        // process
        if ($this->tnotification->run() !== false) {
            $params = [$this->input->post('portal_id')];
            // insert
