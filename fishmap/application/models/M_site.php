...
class M_site extends CI_Model
{
    ...
    function get_parent_group_by_idnav($int_parent, $limit)
    {
        ...
        $query = $this->db->query($sql, [$int_parent]);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
    ...
    function get_user_authority($user_id, $id_group)
    {
        ...
        $query = $this->db->query($sql, [$user_id, $id_group]);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
    ...
    function _displayMenu($params = '1')
    {
        $sql = "SELECT a.*, (SELECT count(*) FROM menu WHERE parent_id = a.id) hasChild FROM menu a 
                LEFT JOIN menu_group b ON a.group_id  = b.id
                WHERE a.parent_id = ? AND b.id = ? ORDER BY position ASC ";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
